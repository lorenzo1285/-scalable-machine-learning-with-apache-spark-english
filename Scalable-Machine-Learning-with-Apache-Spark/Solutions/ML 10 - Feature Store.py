...
# MAGIC 
# MAGIC Check out Feature Store Python API documentation [here](https://docs.databricks.com/dev-tools/api/python/latest/index.html#feature-store-python-api-reference).
# MAGIC 
...
# MAGIC %md
# MAGIC Train a **RandomForestRegressor** model and log the model with feature store. Scope the training to a mlflow run and log testing metrics as well, turn on autolog. 
# MAGIC 
# MAGIC Note that in this example, just for demo purpose, we use the dummy feature "score_diff_from_last_month". As a result, training and testing metrics are not realistic.
